...
namespace RectUtil
{
	...
	Rect CalcLocalRect(const Rect& parentRect, const Vector2& anchorMin, const Vector2& anchorMax, const Vector2& sizeDelta, const Vector2& pivot);
	
	// Given parent rect/pivot and child's anchors/anchorPos/pivot, calculates child's local position in parent's space.
	Vector3 CalcLocalPosition(const Rect& parentRect, const Vector2& parentPivot,
