...
namespace
{
    // Maps CP1251 values 0x80 to 0xFF to the equivalent Unicode Code Points.
    // Mapping is defined here: https://www.unicode.org/Public/MAPPINGS/VENDORS/MICSFT/WINDOWS/CP1251.TXT
    const uint32_t kCp1251CodePointMap[128] = {
...
std::string TextEncode::Cp1251ToUtf8(const std::string& string)
{
    return ExtendedAscii::ToUtf8String(string, kCp1251CodePointMap);
}

uint8_t* TextEncode::Cp1251ToUtf8(uint8_t* inBuffer, uint32_t inBufferSize, uint32_t& outBufferSize)
{
    return ExtendedAscii::ToUtf8(inBuffer, inBufferSize, outBufferSize, kCp1251CodePointMap);
}